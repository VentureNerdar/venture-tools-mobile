    ...
    <VanCellGroup>
      ...
      <VanSwipeCell  v-for="(contact, index) in d.contacts" :key="index" >
        ...
        <template v-else #right>
          <VanButton square type="danger" :text="h.translate('archive')" @click="handleDelete(contact.id)" />
...
const handleDelete = async (contactID: number | undefined) => {
  ...
  try {
    ...
    const consume = useConsumeApi(RoutePaths.CONTACTS, contactID)
    const res = await consume.delete(false)
    if (res) {
      d.contacts = d.contacts.filter((contact:ContactFormModel) => contact.id !== contactID)
    } 
  } catch {
    console.log('error')
  }
