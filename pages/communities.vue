  ...
  <div>
    <VanCellGroup>
      <VanSearch
        show-action
        v-model="searchWord"
        :placeholder="h.translate('search_communities_by_name')"
        @search="handleSearch"
        shape="round"
      >
        <template #action>
      ...
      <div
        v-if="toggleTrash && !d.communities.length"
      ...
      >
        ...
        >
          <VanButton
            ...
            @click="handleRestore(community.id)"
          />
          <VanButton
            square
            type="danger"
            :text="h.translate('destroy')"
            @click="handleDestroy(community.id)"
          />
        </template>
        <template
          v-else
        ...
        >
          <VanButton
            ...
            @click="handleDelete(community.id)"
          />
        </template>
        <VanCell
          :title="community.name || ''"
          @click="toggleTrash ? null : handleEdit(community.id)"
        />
      </VanSwipeCell>
    </VanCellGroup>
  </div>
...
const getCommunities = async () => {
  d.communities = await consume.communities.browse(d.browseOption)
}
...
const handleDelete = async (communityID: number | undefined) => {
  ...
  try {
    ...
    const consume = await useConsumeApi(RoutePaths.COMMUNITIES, communityID)
    const res = await consume.delete(false)
    if (res) {
      d.communities = d.communities.filter(
        (community: CommunityFormModel) => community.id !== communityID
      )
    }
  } catch {
